...
object PartitionPruning {
  ...
  def processData() = {
    ...
    val outputPath = "hdfs://localhost:54310/user/data/demo/randomdata_date_partitioned"
    ...
    clicksDF.repartition(clicksDF("1_date")).write.partitionBy("1_date").parquet(outputPath)
    //  val paritions = 3
    //  clicksDF.coalesce(paritions.toInt).write.partitionBy("1_date").parquet(outputPath)
