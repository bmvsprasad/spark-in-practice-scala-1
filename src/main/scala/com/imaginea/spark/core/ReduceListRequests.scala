...
object ReduceListRequests {
  ...
  def processData(date: String) = {
    val conf = new SparkConf().setAppName("ClickStreamParquet").setMaster("local[*]")
    val sc = new SparkContext(conf)
    val inputPath = "hdfs://localhost:54310/user/data/test/random-data-20160430.txt"
    val outputPath = "hdfs://localhost:54310/user/data/demo/random-data-reduceListRequests"
    ...
    clicksDF.drop("1_date").write.partitionBy("101_mobileOSVersion").parquet(outputPath + s"/date = $date")
